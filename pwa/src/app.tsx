...
import { mkLoadedAsync, mkUnloadedAsync, isLoading, isUnloaded, mkErrorAsync, AsyncState, isLoaded } from './lib/async'
  ...
  return (
    <Router onRoute={route => setState(s2 => !eqRoute(route, s2.route) ? ({...s2, route }) : s2)} >
      ...
      : s1.route.kind === 'projectDetail' && isSome(s1.route.params.slug) ?
        <ProjectPage project={s1.project} onState={a => setState(s2 => ({...s2, project: a(s2.project)}))} />
      : <div>
        <PageHeader>
        </PageHeader>
          <h1>So sorry,</h1>
